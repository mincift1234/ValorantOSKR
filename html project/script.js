...
// Netlify Functions를 통해 Supabase API 키 가져오기
fetch("/.netlify/functions/getSupabaseKeys")
    ...
    .then(data => {
        ...
        loadNotices(); // 공지사항 불러오기
    })
    .catch(error => console.error("Supabase 환경 변수 로드 실패:", error));

// 🔹 공지사항 불러오기 (Supabase에서 데이터 가져오기)
async function loadNotices() {
    ...
    data.forEach((notice) => {
        ...
        listItem.onclick = () => openNoticePopup(notice);
        noticeList.appendChild(listItem);
    });

    // 🔥 관리자 로그인 상태에서만 공지 추가/삭제 UI 표시
...
// 🔹 관리자 로그인 기능 (Supabase Auth 사용)
async function adminLogin() {
    let email = prompt("관리자 이메일을 입력하세요:");
    ...
    loadNotices(); // 공지 UI 업데이트
}

// 🔹 공지 추가 (Supabase에 데이터 저장)
async function addNotice() {
    if (!isAdmin) return alert("관리자만 공지를 추가할 수 있습니다.");

    let title = document.getElementById("notice-title-input").value.trim();
    let content = document.getElementById("notice-content-input").value.trim();
    ...
    if (error) {
        alert("공지 추가 실패: " + error.message);
        return;
    }

    alert("공지사항이 추가되었습니다.");
    loadNotices(); // 새 공지 불러오기
}

// 🔹 공지 삭제 (Supabase에서 개별 공지 삭제)
async function deleteNotice(noticeId) {
    if (!isAdmin) return alert("관리자만 공지를 삭제할 수 있습니다.");

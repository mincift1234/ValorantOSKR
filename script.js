...
async function fetchNotices() {
    let { data, error } = await supabase.from("notice").select("*").order("created_at", { ascending: false });
    ...
    noticeList.innerHTML = ""; // 기존 내용 삭제

    data.forEach((notice) => {
        let li = document.createElement("li");
        li.innerHTML = `<strong>${notice.title}</strong><br>${notice.content}<br><small>${notice.created_at}</small>`;
